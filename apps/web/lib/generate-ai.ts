...
async function runAIGenerationDirect(
	videoId: Video.VideoId,
	userId: string,
): Promise<void> {
	console.log(`[generate-ai] Starting direct AI generation for ${videoId}`);
	...
	if (!query[0]?.video) {
		console.error(`[generate-ai] Video ${videoId} not found`);
		return;
	}
	...
	await db()
		.update(videos)
		.set({ metadata: { ...metadata, aiGenerationStatus: "PROCESSING" } })
		.where(eq(videos.id, videoId));
	...
	try {
		const vtt = await Effect.gen(function* () {
			...
			return yield* s3Bucket.getObject(
		...
		if (Option.isNone(vtt)) {
			console.log(
				`[generate-ai] No VTT found for ${videoId}, marking SKIPPED`,
			);
			await db()
				.update(videos)
		...
		if (text.length < 10) {
			console.log(
				`[generate-ai] VTT too short for ${videoId}, marking SKIPPED`,
			);
			await db()
				.update(videos)
				.set({ metadata: { ...metadata, aiGenerationStatus: "SKIPPED" } })
		...
		let result: AiResult;
		if (chunks.length === 1) {
			result = await generateSingleChunk(segments, videoDuration, groqClient);
		} else {
		...
		console.log(`[generate-ai] Direct AI generation COMPLETE for ${videoId}`);
	} catch (error) {
		console.error(
			`[generate-ai] Direct AI generation ERROR for ${videoId}:`,
			error,
		);
		await db()
			.update(videos)
			.set({ metadata: { ...metadata, aiGenerationStatus: "ERROR" } })
			.where(eq(videos.id, videoId));
	}
}
...
export async function startAiGeneration(
	...
	try {
		...
		runAIGenerationDirect(videoId, userId).catch((error) => {
			console.error(
				`[generate-ai] Unhandled error in direct generation for ${videoId}:`,
				error,
			);
		});
